...
public class Helper {
	...
	public static void printResults(
		...
		double energy = datacenter.getPower() / (3600 * 1000);//能耗，单位：J转kWh
		...
		double slaTimeOfActiveHost = getSlaTimeOfActiveHost(hosts);//返回活跃主机发生SLA违约的违约时间比和曾经活跃过的主机数量

		//SLA违约程度（这里还没取平均<除以N和除以M>）
		//double sla = slaTimePerActiveHost * slaDegradationDueToMigration;
		double sla = (slaTimeOfActiveHost/numberOfHosts) * (slaDegradationDueToMigration/numberOfVms);
		...
		if (outputInCsv) {
			...
			data.append(String.format("%.10f", slaTimeOfActiveHost) + delimeter);
			...
			Log.printLine(String.format("SLA time per active host: %.2f%%", slaTimeOfActiveHost * 100));
	 ...
	 */
	protected static double getSlaTimeOfActiveHost(List<Host> hosts) {
	...
	protected static Map<String, Double> getSlaMetrics(List<Vm> vms) {
		...
		for (Vm vm : vms) {//逐个VM统计
			...
			for (VmStateHistoryEntry entry : vm.getStateHistory()) {//逐个时间片统计
				if (previousTime != -1) {
					double timeDiff = entry.getTime() - previousTime;
					vmTotalAllocated += previousAllocated * timeDiff;//计算一个时间片下，该VM总的MIPS分配量
					vmTotalRequested += previousRequested * timeDiff;//计算一个时间片下，该VM总的MIPS请求量

...
public class CloudletSchedulerTimeShared extends CloudletScheduler {
	...
	/**
	 * Gets the individual MIPS capacity available for each vCPU available for the scheduler,
         * considering that all vCPUs have the same capacity.
	 * 
	 * @param mipsShare list with MIPS share of each vCPU available to the scheduler
	 * @return the capacity of each PE
	 */
	//这个函数返回的实际上是VM中每个vCPU剩余可用MIPs的平均值
	protected double getCapacity(List<Double> mipsShare) {
		double capacity = 0.0;//统计所有vCPU的可用MIPs数
		int cpus = 0;
		...
		//VM中正在运行的所有Cloudlet正在使用的vCPU数，作者假设一个vCPU只能运行一个Cloudlet
		int pesInUse = 0;
		for (ResCloudlet rcl : getCloudletExecList()) {
			pesInUse += rcl.getNumberOfPes();
		}

		//如果所需的vCPU数目多于可用的vCPU数目，则按照可用的vCPU数目来求取平均
	...
	@Override
	//这个其实不太准确，原因和MIPs分配的问题一样，单纯叠加利用率是不合理的（有多个Cloudlet就翻车了）
	//有时间的话，后面用其他数据集测试算法时需要进行修改

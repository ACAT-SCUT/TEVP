 ...
 */
//用于描述Vm的各种信息，每次VM放置和迁移都需要调用相应的Update函数更新这些数据，该类是仿真的核心类之一
public class Vm {
	...
	/** The number of PEs(vCPU) required by the VM. */
	private int numberOfPes;

	/** The  ram capacity of Vm. */
	private int ram;

	/** The bw capacity of Vm. */
	private long bw;
	...
	//下面这四项才是Host在该时间片内真正分配给该VM的资源数量
	...
	private List<Double> currentAllocatedMips;//代码里面没人维护这个变量，所以根本用不了!!
	 ...
	 */
	//返回记录Vm各个vCPU请求的MIPs数的列表，List的大小就是Vm的vCPU数量
	//（这个函数逻辑没问题，但是getCurrentRequestedMips这个函数在一个VM处理多个Cloudlet的情况下并不合理）
	public List<Double> getCurrentRequestedMips() {
		List<Double> currentRequestedMips = getCloudletScheduler().getCurrentRequestedMips();
		if (isBeingInstantiated()) {//初始化时的操作，最初时刻默认Vm的请求量为每个vCPU的MIPs容量
	...
	/**
	 * Gets the current requested max mips among all virtual PEs(vCPU).
	 * 
	 * @return the current requested max mips
	 */
	//返回目前VM各vCPU中，请求到的MIPs数最多的一个vCPU的MIPs数（是实际请求到的MIPs数）
	...
	/**
	 * Gets total CPU utilization percentage of all cloudlets running on this VM at the given time
	 * 
	 * @param time the time

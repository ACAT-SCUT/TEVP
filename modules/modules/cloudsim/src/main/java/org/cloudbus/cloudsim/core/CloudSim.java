...
//DataCenter作为一个SimEntity，创建出来以后直接由CloudSim进行处理，并没有和DatacenterBroker进行绑定，
//CloudSim里面的entities列表存放一次仿真中创建出来的所有实体（当然也包括Datacenter）
//CloudSim里面的entities列表存放一次仿真中创建出来的所有实体（当然也包括Datacenter）
public class CloudSim {
	...
	protected static void initialize() {
		...
		entities = new ArrayList<SimEntity>(); /*一个仿真实体SimEntity列表*/
	...
	public static double run() {
		...
		while (true) { /**直接调用runClockTick()执行仿真涉及的各个操作*/

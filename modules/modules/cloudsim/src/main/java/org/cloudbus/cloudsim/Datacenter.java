...
public class Datacenter extends SimEntity {
     ...
     */
	//每个Datacenter都需要在一个CIS注册，CIS用于管理各种SimEntity，比如Datacenter、Broker、Switch
	private String regionalCisName;

	/** The vm provisioner. */
	//数据中心采用什么vm调度策略（所有Host都用这个策略进行VM调度）
	...
	/** The storage list. */
	//这个列表是各个VM的数据存储的List，用于模拟Datacenter的存储系统
	//但是我们的Power仿真并不会用到这个变量（创建Datacenter的时候，象征性new一个List就完事了）
	private List<Storage> storageList;

	/** The vm list. */
	//Datacenter里面都接收了Broker的哪些Vm
	...
	public Datacenter(
		...
		//给HostList里面的每个host设置Datacenter
		for (Host host : getCharacteristics().getHostList()) {
			host.setDatacenter(this);
		}

		// If this resource doesn't have any PEs then no useful at all
		//获取Host的所有核，我觉得设计的不合理（毕竟都是Host都是异构的，这个单纯就是象征性检查一下）
	...
	@Override
	//这里重写了SimEntity的方法，根据标签event的标签，处理Datacenter的各种事务
	public void processEvent(SimEvent ev) {
		...
		// Resource characteristics inquiry
			...
			//这请求获取Datacenter信息的3个case，在单Datacenter的仿真环境下没啥用
			//多Datacenter环境下，估计是用来给Broker决定Cloudlet分配给哪个数据中心
			...
			//=================================== cloudlet processing ===================================
			// 接下来10个case都是用来处理cloudlet的，包括cloudlet到来时的各种数据更新，
			//以及cloudlet迁移等事务（虽然实际情况下cloudlet很少迁移）
			...
			//=================================== VM processing ===================================
			//下面剩下的这些case全部都是用来处理VM的
			//Boker把构建好的vm传过来Datacenter，然后调用processVmCreate会给服务器调用findHostForVm，完成新VM的放置
	 ...
	 */
	//下面这里两个函数都是用来处理Vm的Data的，我们能耗仿真暂时用不上
	 ...
	 * @post $none
	 */
	//VM最初到达数据中心时，把VM放置到一个Host上，这里我打算采用peap放置算法
	// （具体修改参见allocateHostForVm => findHostForVm）
	protected void processVmCreate(SimEvent ev, boolean ack) {
		...
		if (result) {
			...
			vm.updateVmProcessing(CloudSim.clock(), getVmAllocationPolicy().getHost(vm).getVmScheduler()
					.getAllocatedMipsForVm(vm));//第二个参数是Host会给Vm的每个vCPU分配多少MIPs
		}
	...
	protected void processVmMigrate(SimEvent ev, boolean ack) {
		...
		//destroy VM in src host（到目前为止，彻底摧毁掉源Host上的vm，并维护相关数据<包括MigratingOut列表>）
		getVmAllocationPolicy().deallocateHostForVm(vm);
		//Vm正式迁移到目标Host了，所以从MigratingInVm列表里面删掉，并维护目标Host的相关数据
		//（在决定Vm要迁移，但是迁移事件尚未发生的时候，源Host负担90%的负载，目标Host负担10%的负载）
	 ...
	 * @post $none
	 */
	//Cloudlet从Broker传过来给DataCenter，Broker已经在决定好Cloudlet要分配给哪个Vm
	//这里传过来是为了更新Datacenter关于Cloudlet的数据
	 ...
	 * @post $none
	 */
	//用于更新Datacenter内这一时间片的Vm调度情况（VM迁移策略在这里调用）

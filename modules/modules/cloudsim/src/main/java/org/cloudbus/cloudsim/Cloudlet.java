...
public class Cloudlet {
     ...
     *
     */
    //cloudlet的总任务长度（准确来说应该是计算量），根据CPU的MIPs数决定要运行多久才能完成该cloudlet，单位：MI
     ...
     */
    //Cloudlet每次迁移到VM，并持续到下次迁移或Cloudlet完成，这个时间段的各种时间数据都记录在一个Resource中，
    //resList相当于是记录了Cloudlet历史以来分配给了这么多VM上运行的那些Resource记录
     ...
     * CPU.
     */
    //这个直接决定了cloudlet的CPU利用率是怎么得来的，比如现在用的是PlanetLab的数据集
    // 所以Model也是根据该数据集构造的
     ...
     */
    //每当该Cloudlet放置到某个VM都会创建一个新的Resource对象，当然，Cloudlet迁移也会创建新对象
    //用于记录cloudlet的完成进度（FinishSoFar），在VM的注册时间，运行时间等信息
    ...
    public void setExecParam(final double wallTime, final double actualTime) {
        ...
        final Resource res = resList.get(index);
        //这两个时间不同是因为，一个Cloudlet在VM中有可能会发生PAUSE、CANCEL等操作
     ...
     */
    //若Cloudlet扩展为分配给VM的多个vCPU，且所占比例各不相同的话
    //则这个函数也要改，主要是Cloudlet的UtilizationModelCpu要改
